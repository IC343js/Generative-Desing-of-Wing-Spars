...
//						var u = this.sampleField(x,y, U_FIELD);
    ...
    if (scene.showObstacle) {
        var airfoilPoints = generateNACAAirfoilPoints("23012");
        var xOffset = scene.obstacleX;
        var yOffset = scene.obstacleY;
        var scale = tamaño; // Scale to desired width

        c.fillStyle = scene.showPressure ? "#000000" : "#DDDDDD";
        c.beginPath();
        c.moveTo(cX(xOffset + airfoilPoints[0][0] * scale), cY(yOffset + airfoilPoints[0][1] * scale));

        for (var i = 1; i < airfoilPoints.length; i++) {
            c.lineTo(cX(xOffset + airfoilPoints[i][0] * scale), cY(yOffset + airfoilPoints[i][1] * scale));
        }
    ...
    if (scene.showPressure) {
        var s = "pressure: " + minP.toFixed(0) + " - " + maxP.toFixed(0) + " Pa";
        c.fillStyle = "#000000";
        c.fillText(s, 20, 40);
    }
    }
	...
	function setObstacle(x, y, reset) {
        ...
        var airfoilPoints = generateNACAAirfoilPoints("23012");
        var scale = tamaño; // Adjust scale factor
        var xOffset = x;
        var yOffset = y;
    
        // Scale and translate airfoil points
        var transformedPoints = airfoilPoints.map(function(point) {
            return [
                xOffset + point[0] * scale,
                yOffset + point[1] * scale
            ];
        });
    
        // Function to check if a point is inside the airfoil
    ...
    function generateNACAAirfoilPoints(digits, numPoints = 100) {
        // Digits should be a string like "23012"
        const L = parseInt(digits[0]) * 0.15 / 100;
        const P = parseInt(digits.slice(1, 3)) / 100;
        const t = parseInt(digits.slice(3)) / 100;
        ...
        for (let i = 0; i <= numPoints; i++) {
            ...
            if (x <= P) {
                ...
                yc = (L / ((1 - P) * (1 - P))) * ((1 - 2 * P) + 2 * P * x - x * x);
                dyc_dx = (2 * L / ((1 - P) * (1 - P))) * (P - x);
            }
